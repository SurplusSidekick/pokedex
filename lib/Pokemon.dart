class PokeHub {
  ...
  initBackup() {
    this.backup = this.pokemon;
  }

  restoreFrombackup() {
    this.pokemon = this.backup;
  }

  filterByName(String val) {
    print("NameFilter: ${val}");
    this.pokemon = this.backup.where((pokemon) => pokemon.name.toLowerCase().contains(val.toLowerCase())).toList();

  }

  filterByType(Set filteredTypes) {
    print("TypeFilter: ${filteredTypes}");
    this.pokemon = this.pokemon.where((pokemon) => pokemon.type.toSet().containsAll(filteredTypes)).toList();
  }
}
